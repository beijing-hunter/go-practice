...
func (c *BackendUserController) Edit() {
	...
	//获取关联的课程列表
	var roleIds []string
	userCourseDatas:=models.UserCourseDataListByUserId(Id)

	if len(userCourseDatas)>0{
		for _,item:=range userCourseDatas{
			roleIds = append(roleIds, strconv.Itoa(item.CourseId))
		}
	}
...
func (c *BackendUserController) Save() {
	...
	if models.BackendUserUserNameCount(m.Id,m.UserName){
	...
	//添加课程关系
	models.UserCourseDataDeleteByUserId(m.Id)
	var userCourses []models.UserCourseRef
	for _,courseId:=range m.RoleIds{
		c:=models.UserCourseRef{CourseId:courseId,UserId:m.Id}
		userCourses=append(userCourses,c)
	}

	if len(userCourses)>0{
		o.InsertMulti(len(userCourses), userCourses)
	}
